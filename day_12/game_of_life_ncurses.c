...
void print(int plato[HEIGHT][WIDTH]) {
    for (int i = 0; i < WIDTH + 2; i++) {
        // printw("%s%s%c%s", BACKGROUND_B, BORDER_C, BORDER, RESET);
        printw("%c", BORDER);
    }
    printw("\n");
    for (int i = 0; i < HEIGHT; i++) {
        ...
        for (int j = 0; j < WIDTH; j++) {
            if (plato[i][j])
                // printw("%s%s%c%s", BACKGROUND_A, ALIVE_C, ALIVE_O, RESET);
                printw("%c", ALIVE_O);
            else
                // printw("%s%s%c%s", BACKGROUND_D, DEAD_C, DEAD_O, RESET);
                printw("%c", DEAD_O);
        }
        // printw("%s%s%c%c%s", BACKGROUND_B, BORDER_C, BORDER, '\n', RESET);
        printw("%c%c", BORDER, '\n');
    }
    for (int i = 0; i < WIDTH + 2; i++) {
        // printw("%s%s%c%s", BACKGROUND_B, BORDER_C, BORDER, RESET);
